...
class Controller:
    ...
    def get_photo(self, link, link_name, relative_link, section):
        ...
        if not search_sequence.startswith('http') and ('Main Navigation' in section or 'accordion' in link_name or 'tab' in link_name):
            print('splitting')
            try:
                first_number_index = self.find_first_number_index(search_sequence)
        ...
        if search_word.startswith('http') or search_word.startswith('/') or ' ' in search_word:
        ...
        while True:
            ...
            try:
                ...
                elements2=[]
                try:
                    elements1 = driver.find_elements(By.XPATH, f"//*[contains(text(), '{search_word}')]")
