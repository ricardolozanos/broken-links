    ...
    try:
        ...
        for cell in sheet[column_with_links]:
            if cell.value:
                ...
                page_progress = Progressbar(root, variable=page_progress_var, maximum=len(page_links), mode='determinate')
                page_progress.pack()
...
def update_page_progress(current_page, total_pages):
    page_progress_label.config(text=f"{current_page}/{total_pages} Pages")

def update_link_progress(current_link, total_links):
    link_progress_label.config(text=f"{current_link}/{total_links} Links")
...
def check_links_thread():
    ...
    start_link_checking()
    progress_bar.stop()
    page_progress_label.config(text="")
    link_progress_label.config(text="")
...
page_progress_label = tk.Label(main_frame, text="", font=('Helvetica', 12))
page_progress_label.pack()

link_progress_label = tk.Label(main_frame, text="", font=('Helvetica', 12))
