        ...
        try:
            ...
            for link_cell, template_cell in zip(sheet[column_with_links], sheet[column_with_templates]):
                if link_cell.value:
                    ...
                    page_progress = Progressbar(self.root, variable=page_progress_var, length=300,maximum=len(page_links), mode='determinate')
                    page_progress.pack()

                    # Update page progress label
                    self.update_page_progress(total_links_checked, total_links)
                    ...
                    for i, page_link in enumerate(page_links):
                        ...
                        self.update_link_progress(i + 1, len(page_links))
